...
def interleaved_sum(n, odd_func, even_func):
    ...
    """
    "*** YOUR CODE HERE ***"
    def recursive(m, is_odd):
        if m > n:
            return 0
        elif is_odd:
            return odd_func(m) + recursive(m + 1, not is_odd)
        else:
            return even_func(m) + recursive(m + 1, not is_odd)
    return recursive(1, True)
...
def make_anonymous_factorial():
    ...
    return (lambda f: (lambda x:x(x))(lambda x:f(lambda *argus:x(x)(*argus))))(lambda f: lambda x : 1 if x == 1 else x * f(x - 1)) 