def shuffle(s):
    ...
    "*** YOUR CODE HERE ***"
    half_length = len(s) // 2
    return [item for pairs in zip(s[:half_length], s[half_length:]) for item in pairs]
...
def balanced(m):
    ...
    balanced_left = balanced(end(left(m)))
    balanced_right = balanced(end(right(m)))

    if not balanced_left or not balanced_right:
        return False


    mass_left = total_mass(end(left(m)))
    mass_right = total_mass(end(right(m)))

    return length(left(m)) * mass_left == length(right(m)) * mass_right
...
def max_path_sum(t):
    ...
    def recursive(t, sum):
        ...
        for branch in branches(t):
            recursive(branch, sum + label(branch))
